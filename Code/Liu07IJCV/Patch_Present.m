...
intervalSize = 4;
Cons_a = zeros(320,240);
xbegin = 4; ybegin = 4;
mm = 0;
for m = ybegin:intervalSize:320-2
    nn = 0;
    mm = mm +1;
    for n = xbegin:intervalSize:240-2
        nn = nn + 1;
        Cons_a (m-3:m+2,n-3:n+2) = ...
    reshape(P_b(ann(sub2ind([79,59],mm,nn)),:),[6,6]);        
    %reshape(P_b(sub2ind([79,59],mm,nn),:),[6,6]);
    end
