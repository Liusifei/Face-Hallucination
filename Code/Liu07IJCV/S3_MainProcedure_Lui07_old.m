...
for fileidx=1:filecount   %this is the index of test images, irrelavant to training images
    %open specific file
...
%     rm = rm(1:Samplenum);
    ...
    for m = 1:size(middlebandimage,3)
        ...
        %     for m = 1:3
%                 [D_best,ann] = PatchMatch(img_middleband,middlebandimage(:,:,rm(m)),itern);
        ...
        B = uint8(repmat(middlebandimage(:,:,m),[1,1,3])*255);
        ann = nnmex(A, B, algo, 7, [], [], [], [], [], cores, [], [], [], [], [], itern);
        % Match original output: 3 * 79* 59****************
        D_best = reshape(ann(:,:,3,:),[size(ann,1),size(ann,2),3]);
...
%             indxy = ann(:,:,:,kk);
        ...
        if m == 1
            ...
            Cnn = zeros(itern*Samplenum, size(D_best,2), size(D_best,3));
...
%     DrawRandPatch(Cnn);
    ...
    CH = zeros(kn,kn,size(ann,2),size(ann,3)-1);
    CV = zeros(kn,kn,size(ann,2)-1,size(ann,3));
    ...
    figure(3);
    saveas(gcf,[dst,'SPImage12_',sprintf('%.4d',fileidx),'.fig'],'fig');
    SP_IMG = (img_middleband + CosLocalIm);
    save([dst,'CosLocalIm_',sprintf('%.4d',fileidx),'.mat'],'CosLocalIm');
    SP_IMG = (SP_IMG-min(SP_IMG(:)))/(max(SP_IMG(:))-min(SP_IMG(:)));
    
    %----------------------------------------------------------------------
    % Step 3, Biliteral Filtering
    %----------------------------------------------------------------------
    SP_IMG = bfilter2(SP_IMG,2,[1.2,0.1]);
    figure(4);clf;
    imshow(SP_IMG);
    imwrite(SP_IMG,[dst,'SPIMG_',sprintf('%.4d',fileidx),'.png']);
