...
for fileidx=1:length(filelist)   %this is the index of test images, irrelavant to training images
    %open specific file
    ...
    img_y = img_yiq(:,:,1);
%     IQLayer = img_yiq(:,:,2:3);
    IQLayer_upsampled = imresize(img_yiq,para.zooming);
...
%         ann = nnmex(A, CB, algo, 7, [], [], [], [], [], cores);   % Warm up
...
%         Cons_l = Patch_Present(L_Patches, ann);
    ...
    IQLayer_upsampled(:,:,1) = SP_IMG;
    im = YIQ2RGB(IQLayer_upsampled);
    figure(4);
    imshow(im);
    imwrite(im,[dst,fn_testfile_short,'.png']);
