%Chih-Yuan Yang
%10/29/12
%Slightly modify it to save dictionary in each iteration
function [B S stat] = F3_sparse_coding(X_total, num_bases, beta, sparsity_func, epsilon, num_iters, batch_size, fname_save, pars, Binit, resample_size)
...
while t < pars.num_trials
    ...
    for batch=1:(size(X,2)/pars.batch_size),
        ...
        folder_save = fullfile('Data','Upfrontal3','s4','Dictionary_test1');
        U22_makeifnotexist(folder_save);
        fn_save = sprintf('B_iter%03d.mat',t);
        save(fullfile(folder_save,fn_save),'B');
    end
