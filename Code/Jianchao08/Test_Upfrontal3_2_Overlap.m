...
para.settingnote = '';
para.tuning = 2;
para.tuningnote = 'Implement the overlap control mentioned in ICIP08 Yang but not implement in his released code';
...
for fileidx=fileidx_start:fileidx_end
    ...
    %this is a special format required by F3_L1SR,
    %if the input is img_y_double ranged from 0~1, the norm will be too small and the returned coef w will be empty
    img_y_255_double = double(rgb2gray(img_lr_rgb_uint8));
    img_out_255_double = F3a_L1SR(img_y_255_double, zooming, patch_size, overlap, Dh, Dl, lambda, regres);
