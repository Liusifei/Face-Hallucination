...
sf = 4;
Gau_sigma = 1.6;
folder_save = fullfile('Result','Test2_PubFig_HighContrast');
...
for idx_file_test=idx_file_start:idx_file_end
    ...
    modelname = 'mi';
    bs = [];
    %The UCI algorithm only accepts color images
    [bs, posemap] = F2_ReturnLandmarks(img_bb, modelname);
    
    %some detected faces are wrong, due to the illumination
    %The variable c is used to determine the pose with another variable posemap.
    %If multiple faces are detected, bs will contain multiple instances.
    c = bs(1).c;    %we assume there is only one face in the image.
    val_pose = posemap(c);
    bs_used = bs(1);
    landmarks_test = F4_ConvertBStoMultiPieLandmarks(bs_used);
    %show the landmarks to check the effectiveness of the localization algorithm
    bshownumber = true;
    bdrawpose = true;
    bvisible = true;
    str_pose = sprintf('%d',val_pose);
    hfig = U21b_DrawLandmarks_Points_ReturnHandle(img_bb,landmarks_test,str_pose,bshownumber,bdrawpose,bvisible);
    %how to solve the problem if landmarks are wrongly localized?
    keyboard
    close(hfig);
    
    %This step can be further improved by the ICCV 13 method in order to generate better image quality and save time.
    %Task: replace this step by the ICCV 13 algorithm
