%Chih-Yuan Yang
%11/19/12
%Fully automatically detect, align, create groundtruth, crop, downsample, upsample, re-detect, transfer details,
%I am still tring to develep it, but not finished
clc
clear
close all
...
folder_pubfig = fullfile(folder_code,'PubFig','Wild2','FullImage');
para.setting = 3;
para.settingnote = 'Fully automatically detect, align, create groundtruth, crop, downsample, upsample, re-detect, transfer details';
para.tuning = 1;
para.tuningnote = '';
legend = 'Ours';
model = 'mi';
folder_detectedlandmark = fullfile('Temp','DetectedLandmarks','Wild3');
glasslistfolder = fullfile('Examples','Upfrontal3');
folder_load = fullfile('Examples','Upfrontal3','PreparedMatForLoad');       %example images
fn_load = 'ExampleDataForLoad.mat';

fileidx_start = 1;
fileidx_end = 'all';
busesymphonyfile = true;
symphonyfilename = 'symphonyfile.txt';
brecreatesymphonyfile = false;
filluntil = 0;
...
para = U23a_PrepareResultFolder('Result', para);

%load all training images
loaddata = load(fullfile(folder_load,fn_load),'exampleimages_hr','exampleimages_lr','landmarks');
...
folder_finalsave = fullfile(para.tuningfolder, 'GeneratedImages');
U22_makeifnotexist(folder_finalsave);
...
filelist = dir(fullfile(folder_pubfig, '*.jpg'));
...
%symphonyfile
if busesymphonyfile
    fn_symphonyfull = fullfile(io.tuningfolder,io.symphonyfilename);
    if ~exist(fn_symphonyfull,'file') || brecreatesymphonyfile
        filenamelist = cell(filecount,1);
        for i=1:filecount
            filenamelist{i} = filelist(i).name;
        end
        U27_CreateSymphonyFile(fn_symphonyfull,filecount,filenamelist,filluntil);
    end
end
...
for fileidx=fileidx_start:fileidx_end
    ...
    %use try to control here, because it may fail
    try
        img_pubfig = im2double(imread(fullfile(para.folder_pubfig,fn_test)));
    catch err
        fprintf('can not load the file\n');
        continue
    end
    img_yiq = RGB2YIQ(img_pubfig);
    %Here, stop 11/18/12 4:16pm, I need to go back to handle the supplementary material at first
    img_y = img_yiq(:,:,1);
    IQLayer = img_yiq(:,:,2:3);
...
%            folder_save = fullfile(folder_project,'PaperWriting','CVPR13','manuscript','figs','Illustration','FlowChart2');
...
%        end
    ...
    fn = sprintf('%s_%s_%d_%d.png',fn_testshort,legend,para.setting,para.tuning);
    imwrite(img_out_rgb, fullfile(folder_finalsave,fn));
    
    %save data
    gradient_final = gradientwholeimage;
    fn_save = sprintf('%s_%s_%d_%d_data.mat',fn_testshort,legend,para.setting,para.tuning);
    save(fullfile(folder_finalsave,fn_save),'mask_hr_record','mask_lr_record','retrievedidxrecord',...
