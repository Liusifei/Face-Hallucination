%Chih-Yuan Yang
%09/12/13
%Modify TestPubFig to TestJANUS, there are a few differences.
%First, we are unable to generate the groundtruth images for JANUS dataset because some resolutiion of the facial
%regions are too low.
%Second, we can not manually select partial good resutls
%JANUS2: I cropped some LR images using PP2f, test the SR results
clc
clear
close all
...
sf = 4;
Gau_sigma = 1.6;
folder_save = fullfile('Result','JANUSProposal');
folder_test = fullfile('Source','JANUSProposal','Input');
folder_landmark_detected = fullfile('Landmarks','Detected','JANUSProposal2_Ready');
fn_filelist_test = 'JANUSProposalLR_detectable.txt';
str_legend = 'Ours';
%folder_semaphore = fullfile('Temp','Semaphore');
%folder_semaphore = '\\vision-U1\Research\120801SRForFace\code\Ours2\Temp\Semaphore';
...
for idx_file_test=idx_file_start:idx_file_end
    ...
    [gradients_texture, img_texture, img_texture_backprojection] = ...
        F37g_GetTexturePatchMatch_Aligned(img_y, rawexampleimage, allLRexampleimages, landmarks_test, rawexamplelandmarks);
