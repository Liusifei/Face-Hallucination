...
%PP17a: try to find the reason why it does not work
...
for j=imageidxstart:imageidxend
    ...
    lefteyecenter = mean(landmarks_test(37:42,:));
    righteyecenter = mean(landmarks_test(43:48,:));
    twoeyecenter = (lefteyecenter + righteyecenter)/2;
    %face_left = round(twoeyecenter(1)) - 120;
    %face_right = round(twoeyecenter(1)) + 119;
    %face_top = round(twoeyecenter(2)) - 160;
    %face_bottom = round(twoeyecenter(2)) + 159;
    face_left_lr = round(twoeyecenter(1)/zooming) - round(120/zooming);
    face_right_lr = round(twoeyecenter(1)/zooming) + round(120/zooming)-1;
    face_top_lr = round(twoeyecenter(2)/zooming) - round(160/zooming);
    face_bottom_lr = round(twoeyecenter(2)/zooming) + round(160/zooming)-1;
    ...
    for i=1:wlength
        ...
        l2normaverage(i) = mean2(l2normrecord(face_top_lr:face_bottom_lr,face_left_lr:face_right_lr));
    end
