%Chih-Yuan Yang
%09/19/12
%load allLRexampleimages to speed up
function gradients_merge = F16c_ACCV12TextureGradientNotIntensity(img_y, zooming, Gau_sigma ,sfall,srecall,allHRexampleimages,allLRexampleimages)
    if zooming == 4
        para.Gau_sigma = 1.6;
    elseif zooming == 3
        para.Gau_sigma = 1.2;
...
%    img_texture_backproject = F11_BackProjection(img_y, img_texture, Gau_sigma, iternum);
    ...
    %here is a bug, this is the sum of gradients rather than weighted average of gradients,
    %so that the summed gradients may be greater than 1
