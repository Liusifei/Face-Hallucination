...
para.testimagefolder = fullfile('Source','NonUpfrontal4_Compressed','25');
para.savefolder = 'GeneratedImages';
para.setting = 4;
para.settingnote = 'Compressed input image';
para.tuning = 1;
para.tuningnote = 'Compression parameter 25';
para.legend = 'cvpr13';
para.model = 'mi';
para.detectedlandmarkfolder = fullfile('Temp','DetectedLandmarks','NonUpfrontal3');
para.glasslistfolder = fullfile('Examples','NonUpfrontal3');
para.loadfolder = fullfile('Examples','NonUpfrontal3','PreparedMatForLoad');
...
%     bs = loaddata.bs(1);
    ...
    %Now there is a problem. Due to the severe distoration, the detected
    %facial landmarks are no more the standard 68 points.
    %Thus the prepared exemplar images can not be used.
    if size(bs.xy,1) < 68
        continue
    end
