function [D_best,ann] = PatchMatch_Singlepatch(Patch_a,IM_b,itern)

global SIZE; global PATCHSIZE; global INTERVALSIZE;
SIZE = size(IM_a);
patchSize = 4; PATCHSIZE = patchSize;
intervalSize = patchSize/2; INTERVALSIZE = intervalSize;
global MAX_X; 
global MAX_Y;
...
% fprintf('\n');
ann = reshape(ann(end,:,:),[max_y,max_x]);
D_best = reshape(D_best(end,:,:),[max_y,max_x]);
%
%------------for debug--------------
% Cons_a = Patch_Present(Patches_b, reshape(ann,[max_y,max_x]));
% figure;
% plot(D);