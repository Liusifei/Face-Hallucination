...
% reconstruct through examplehr, with index ann;
function Cons_a = Patch_Present_SR_t(examplehr, ind_size, factor)
...
for m = ybegin+1:intervalSize:size(1)-boundarySize+1
    ...
    for n = xbegin+1:intervalSize:size(2)-boundarySize+1
        ...
        Cons_a(m-boundarySize:m+boundarySize-1,n-boundarySize:n+boundarySize-1)...
            = double(reshape(examplehr(sub2ind(ind_size,mm,nn),:),[PATCHSIZE * factor,PATCHSIZE *factor]))+...
            Cons_a(m-boundarySize:m+boundarySize-1,n-boundarySize:n+boundarySize-1);
    end
end
Cons_a = Cons_a/4;
